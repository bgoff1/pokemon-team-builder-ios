...
class CollectionViewController: UICollectionViewController {
    ...
    struct Pokemon {
        ...
        init (_ diction: NSDictionary) {
            self.name = diction["name"] as! String
            self.types = diction["types"] as! [String]
            self.games = diction["games"] as! [String]
            self.generation = diction["generation"] as! Int
            self.image = diction["image"] as! String
            self.number = diction["pokemonNumber"] as! Int
        }
        let name: String
        let types: [String]
        let games: [String]
        ...
        if let path = Bundle.main.path(forResource: "pokemon", ofType: "json") {
            do {
                ...
                if let jsonResult = jsonResult as? Dictionary<String, AnyObject> {
                    ...
                    if let person = jsonResult["pokemon"] as? [AnyObject] {
                        for poke in person {
                            let poke1 = Pokemon.init(poke as! NSDictionary)
                            pokemon.append(poke1)
                        }
                    }
                }
            } catch {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath)
            ...
            imageView.image = temp(pokemon[indexPath.row].image)
            cell.addSubview(imageView)
        
        cell.sub
        
        print("hi \(pokemon[indexPath.row].name)")
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print ("User tapped on pokemon \(pokemon[indexPath.row].name)")
    }
